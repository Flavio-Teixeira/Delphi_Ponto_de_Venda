object Usuario00: TUsuario00
  Left = 207
  Top = 107
  BorderStyle = bsDialog
  Caption = 'Cadastro de Usu'#225'rios'
  ClientHeight = 334
  ClientWidth = 477
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 5
    Top = 285
    Width = 466
    Height = 41
    TabOrder = 1
    object BT_Sair: TBitBtn
      Left = 376
      Top = 8
      Width = 75
      Height = 25
      Hint = 'Retornar ao Menu Principal|Retornar ao Menu Principal'
      Caption = 'Sair'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = BT_SairClick
      Glyph.Data = {
        
        0333337FFFFFFFFF7F3333000000000003333377777777777333}
      NumGlyphs = 2
    end
    object BT_Incluir: TBitBtn
      Left = 196
      Top = 8
      Width = 75
      Height = 25
      Hint = 'Cadastrar Novo Usu'#225'rio|Cadastrar Novo Usu'#225'rio'
      Caption = 'Incluir'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnClick = BT_IncluirClick
      Glyph.Data = {
        
        5990555555555555577755555555555555555555555555555555}
      NumGlyphs = 2
    end
    object DB_Usuario_Nome: TDBEdit
      Left = 7
      Top = 5
      Width = 8
      Height = 19
      Ctl3D = False
      DataField = 'Usuario_Nome'
      DataSource = ConexaoBD.DS_Usuarios
      ParentCtl3D = False
      TabOrder = 2
      Visible = False
    end
    object DB_Usuario_Senha: TDBEdit
      Left = 14
      Top = 5
      Width = 9
      Height = 19
      Ctl3D = False
      DataField = 'Usuario_Senha'
      DataSource = ConexaoBD.DS_Usuarios
      ParentCtl3D = False
      TabOrder = 3
      Visible = False
    end
    object DB_Usuario_Fiscal: TDBEdit
      Left = 22
      Top = 5
      Width = 9
      Height = 19
      Ctl3D = False
      DataField = 'Usuario_Fiscal'
      DataSource = ConexaoBD.DS_Usuarios
      ParentCtl3D = False
      TabOrder = 4
      Visible = False
    end
    object DB_Usuario_Cor_Grid: TDBEdit
      Left = 30
      Top = 5
      Width = 9
      Height = 19
      Ctl3D = False
      DataField = 'Usuario_Cor_Grid'
      DataSource = ConexaoBD.DS_Usuarios
      ParentCtl3D = False
      TabOrder = 5
      Visible = False
    end
    object DB_Usuario_Cor_Letra: TDBEdit
      Left = 38
      Top = 5
      Width = 9
      Height = 19
      Ctl3D = False
      DataField = 'Usuario_Cor_Letra'
      DataSource = ConexaoBD.DS_Usuarios
      ParentCtl3D = False
      TabOrder = 6
      Visible = False
    end
    object DB_Usuario_Gaveta: TDBEdit
      Left = 46
      Top = 5
      Width = 9
      Height = 19
      Ctl3D = False
      DataField = 'Usuario_Gaveta'
      DataSource = ConexaoBD.DS_Usuarios
      ParentCtl3D = False
      TabOrder = 7
      Visible = False
    end
    object DB_Usuario_Impressora: TDBEdit
      Left = 54
      Top = 5
      Width = 9
      Height = 19
      Ctl3D = False
      DataField = 'Usuario_Impressora'
      DataSource = ConexaoBD.DS_Usuarios
      ParentCtl3D = False
      TabOrder = 8
      Visible = False
    end
  end
  object Panel2: TPanel
    Left = 5
    Top = 5
    Width = 466
    Height = 273
    TabOrder = 0
    object GroupBox1: TGroupBox
      Left = 8
      Top = 64
      Width = 450
      Height = 201
      Caption = '  Resultados Obtidos  '
      TabOrder = 1
      object GridUsuarios: TDBGrid
        Left = 9
        Top = 14
        Width = 432
        Height = 179
        Ctl3D = False
        DataSource = ConexaoBD.DS_Usuarios
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentCtl3D = False
        ParentFont = False
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = [fsBold]
        OnCellClick = GridUsuariosCellClick
        Columns = <
          item
            Expanded = False
            FieldName = 'Usuario_Nome'
            Title.Alignment = taCenter
            Title.Caption = 'Nome'
            Width = 400
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'Usuario_Senha'
            Title.Caption = 'Senha'
            Visible = False
          end>
      end
    end
    object GroupBox2: TGroupBox
      Left = 9
      Top = 8
      Width = 448
      Height = 49
      Caption = '  Busca  '
      TabOrder = 0
      object Label1: TLabel
        Left = 24
        Top = 21
        Width = 49
        Height = 13
        Caption = 'buscar em'
      end
      object BT_Buscar: TBitBtn
        Left = 360
        Top = 16
        Width = 81
        Height = 25
        Caption = '&Buscar'
        TabOrder = 2
        OnClick = BT_BuscarClick
        Glyph.Data = {
          
          888888800088800088888888888888888888}
      end
      object ComboLocalBusca: TComboBox
        Left = 88
        Top = 18
        Width = 89
        Height = 21
        BevelInner = bvNone
        BevelKind = bkSoft
        Style = csDropDownList
        Enabled = False
        ItemHeight = 13
        ItemIndex = 0
        TabOrder = 0
        Text = 'Nome'
        Items.Strings = (
          'Nome')
      end
      object EditBusca: TMaskEdit
        Left = 192
        Top = 18
        Width = 153
        Height = 19
        Ctl3D = False
        ParentCtl3D = False
        TabOrder = 1
        OnKeyPress = EditBuscaKeyPress
      end
    end
  end
end
