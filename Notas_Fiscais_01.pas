unit Notas_Fiscais_01;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, StdCtrls, Buttons, ExtCtrls, Mask, Grids, DBGrids, Gauges, xmldom,
  XMLIntf, msxmldom, XMLDoc, DelphiZXingQRCode;
  
type
  TNotasFiscais01 = class(TForm)
    GroupBox1: TGroupBox;
    Label2: TLabel;
    Label3: TLabel;
    ...
    Label5: TLabel;
    Label6: TLabel;
    ValorTotalNota: TLabel;
    DBGrid_Selecao: TDBGrid;
    BTN_Emite_Nota: TBitBtn;
    Label7: TLabel;
    Rel_Valor: TEdit;
    Label8: TLabel;
    ValorTotalProcura: TLabel;
    OcultaEmitidas: TCheckBox;
    XML_Gerado: TEdit;
    ...
    procedure DBGrid_SelecaoDrawColumnCell(Sender: TObject;
      const Rect: TRect; DataCol: Integer; Column: TColumn;
      State: TGridDrawState);
    procedure BTN_Emite_NotaClick(Sender: TObject);
    procedure DBGrid_SelecaoDblClick(Sender: TObject);
    procedure Rel_ValorKeyPress(Sender: TObject; var Key: Char);
  private
...
var
  NotasFiscais01: TNotasFiscais01;
...
function TNotasFiscais01.completa_zeros(texto: String; tamanho: Integer): String;
...
function TNotasFiscais01.retira_acentos(texto:String; tamanho:Integer): String;
...
procedure TNotasFiscais01.FormShow(Sender: TObject);
begin
...
procedure TNotasFiscais01.Rel_Periodo_InicialKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TNotasFiscais01.Rel_Periodo_FinalKeyPress(Sender: TObject;
  var Key: Char);
begin
    So_Numero(Sender, Key);
...
procedure TNotasFiscais01.BT_SairClick(Sender: TObject);
begin
    NotasFiscais01.Close;
end;

procedure TNotasFiscais01.MaquinasKeyPress(Sender: TObject;
  var Key: Char);
begin
    If Key = #13 Then
      Begin
...
procedure TNotasFiscais01.BT_ProcurarClick(Sender: TObject);
...
begin
     If StrToDate(Rel_Periodo_Inicial.Text)<=StrToDate(Rel_Periodo_Final.Text) Then
        ...
        Comando_SQL := 'Select SUM(Corpo_Cupom_Vlr_Total) AS Valor_Total_Procura from Cabeca_Cupons,Corpo_Cupons Where ';
        ...
        If Trim(Rel_Valor.Text) <> '' Then
           Begin
           Comando_SQL := Comando_SQL + 'AND Corpo_Cupom_Vlr_Total >= ' + Virgula_Ponto(Trim(Rel_Valor.Text)) + ' ';
        End;

        If OcultaEmitidas.Checked = True Then
           Begin
           Comando_SQL := Comando_SQL + 'AND Cabeca_Cupom_Cliente_Status = ' +#39+ 'Aguardando' +#39+ ' ';
        End;

        Comando_SQL := Comando_SQL + 'Order By Cabeca_Cupom_Data_Emissao ASC, Cabeca_Cupom_Numero ASC';

        //*** Seleciona os Produtos para a Nota Fiscal ***
        ...
        Vlr_Exibicao := Trim(Virgula_Ponto(ConexaoBD.SQL_Compras_Clientes.FieldByName('Valor_Total_Procura').Text));
        Val(Vlr_Exibicao,Vlr_Numero,Erro);
        Str(Vlr_Numero:13:2,Vlr_Exibicao);
        Vlr_Exibicao := Ponto_Virgula(Trim(Vlr_Exibicao));

        ValorTotalProcura.Caption := Vlr_Exibicao;

        If Trim(ValorTotalProcura.Caption) = '' Then
           Begin
           ValorTotalProcura.Caption := '0,00';
        End;  
        ...
        //*** Atualiza o Grid de Produtos Selecionados ***
        ConexaoBD.SQL_Selecao_Produtos_Nota.Close;
        ConexaoBD.SQL_Selecao_Produtos_Nota.SQL.Clear;
        ConexaoBD.SQL_Selecao_Produtos_Nota.SQL.Add('SELECT SUM(Selecao_Produto_Nota_Vlr_Total) AS Selecao_Produto_Total_Nota_Fiscal FROM Selecao_Produtos_Nota');
        ConexaoBD.SQL_Selecao_Produtos_Nota.Open;

        Vlr_Exibicao := Trim(Virgula_Ponto(ConexaoBD.SQL_Selecao_Produtos_Nota.FieldByName('Selecao_Produto_Total_Nota_Fiscal').Text));
        Val(Vlr_Exibicao,Vlr_Numero,Erro);
        Str(Vlr_Numero:13:2,Vlr_Exibicao);
        Vlr_Exibicao := Ponto_Virgula(Trim(Vlr_Exibicao));

        ValorTotalNota.Caption := Vlr_Exibicao;

        If Trim(ValorTotalNota.Caption) = '' Then
           Begin
           ValorTotalNota.Caption := '0,00';
        End;

        //*** Atualiza o Grid de Produtos Selecionados ***
        ConexaoBD.SQL_Selecao_Produtos_Nota.Close;
        ConexaoBD.SQL_Selecao_Produtos_Nota.SQL.Clear;
        ConexaoBD.SQL_Selecao_Produtos_Nota.SQL.Add('SELECT * FROM Selecao_Produtos_Nota');
        ConexaoBD.SQL_Selecao_Produtos_Nota.Open;
...
procedure TNotasFiscais01.DBGrid_PedidosDrawColumnCell(
  Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
     If (DBGrid_Pedidos.Fields[8].Text = 'Aguardando') Then
        Begin
...
procedure TNotasFiscais01.DBGrid_PedidosDblClick(Sender: TObject);
...
begin
     If (DBGrid_Pedidos.Fields[0].Text <> '') AND (DBGrid_Pedidos.Fields[8].Text = 'Aguardando') Then
        Begin
        Ampulheta();
        ...
        ConexaoBD.SQL_Selecao_Produtos_Nota.Close;
        ConexaoBD.SQL_Selecao_Produtos_Nota.SQL.Clear;
        ConexaoBD.SQL_Selecao_Produtos_Nota.SQL.Add('SELECT * FROM Selecao_Produtos_Nota WHERE Selecao_Produto_Nota_Cabeca = ' + #39+ Trim(DBGrid_Pedidos.Fields[0].Text) +#39+ ' AND Selecao_Produto_Nota_Prod_Codigo = ' + #39+ Trim(DBGrid_Pedidos.Fields[3].Text) +#39);
        ConexaoBD.SQL_Selecao_Produtos_Nota.Open;

        If ConexaoBD.SQL_Selecao_Produtos_Nota.Eof = True Then
           Begin
           //*** Registra o Produto se Não Existir ***
           Comando_SQL := 'INSERT INTO Selecao_Produtos_Nota(';
           Comando_SQL := Comando_SQL + 'Selecao_Produto_Nota_Cabeca, ';
           Comando_SQL := Comando_SQL + 'Selecao_Produto_Nota_Data_Emissao, ';
           Comando_SQL := Comando_SQL + 'Selecao_Produto_Nota_Hora_Emissao, ';
           Comando_SQL := Comando_SQL + 'Selecao_Produto_Nota_Prod_Codigo, ';
           Comando_SQL := Comando_SQL + 'Selecao_Produto_Nota_Prod_Descricao, ';
           Comando_SQL := Comando_SQL + 'Selecao_Produto_Nota_Prod_Qtde, ';
           Comando_SQL := Comando_SQL + 'Selecao_Produto_Nota_Vlr_Unitario, ';
           Comando_SQL := Comando_SQL + 'Selecao_Produto_Nota_Vlr_Total) ';
           Comando_SQL := Comando_SQL + 'VALUES(';
           Comando_SQL := Comando_SQL + #39+ Trim(DBGrid_Pedidos.Fields[0].Text) +#39+ ', ';
           Comando_SQL := Comando_SQL + #39+ Inverte_Data_Plus(Trim(DBGrid_Pedidos.Fields[1].Text),'amd','-') +#39+ ', ';
           Comando_SQL := Comando_SQL + #39+ Trim(DBGrid_Pedidos.Fields[2].Text) +#39+ ', ';
           Comando_SQL := Comando_SQL + #39+ Trim(DBGrid_Pedidos.Fields[3].Text) +#39+ ', ';
           Comando_SQL := Comando_SQL + #39+ Trim(DBGrid_Pedidos.Fields[4].Text) +#39+ ', ';
           Comando_SQL := Comando_SQL + #39+ Trim(DBGrid_Pedidos.Fields[5].Text) +#39+ ', ';
           Comando_SQL := Comando_SQL + #39+ Virgula_Ponto(Trim(DBGrid_Pedidos.Fields[6].Text)) +#39+ ', ';
           Comando_SQL := Comando_SQL + #39+ Virgula_Ponto(Trim(DBGrid_Pedidos.Fields[7].Text)) +#39+ ') ';
...
procedure TNotasFiscais01.DBGrid_SelecaoDrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
    DBGrid_Selecao.Canvas.Brush.Color := clMoneyGreen;
    DBGrid_Selecao.DefaultDrawDataCell(Rect, DBGrid_Selecao.columns[datacol].field, State);
end;

procedure TNotasFiscais01.BTN_Emite_NotaClick(Sender: TObject);
...
begin
     If ((DBGrid_Selecao.Fields[0].Text <> '') And (Trim(BTN_Emite_Nota.Caption) = 'Nota Fiscal')) Then
        Begin
        Ampulheta();
        ...
        If Imprime_Nota = True  Then
           ...
           //*** Prepara o Nome do XML Gerado ***
           XML_Gerado.Text := 'CFe_' + Trim(Numero_Nota) + '.xml';
           ...
           Valor_Total := StrToFloat(Ponto_Virgula(Trim(ValorTotalNota.Caption)));

           While Not ConexaoBD.SQL_Selecao_Produtos_Nota.Eof Do
                 Begin
                 Sequencia_Item := Sequencia_Item + 1;

                 Tributos_Unitario := ((StrToFloat(Ponto_Virgula(Trim(ConexaoBD.SQL_Selecao_Produtos_Nota.FieldByName('Selecao_Produto_Nota_Prod_Qtde').Text))) * (StrToFloat(Ponto_Virgula(Trim(ConexaoBD.SQL_Selecao_Produtos_Nota.FieldByName('Selecao_Produto_Nota_Vlr_Unitario').Text)))) * 5.47) / 100);
                 Tributos_Total    := Tributos_Total + Tributos_Unitario;

                 //*** Seleciona o Produto ***
                 Comando_SQL := 'SELECT * FROM Produtos WHERE Produto_Codigo = ' + Trim(ConexaoBD.SQL_Selecao_Produtos_Nota.FieldByName('Selecao_Produto_Nota_Prod_Codigo').Text);
                 ...
                 Grava_Linha := Grava_Linha + '<qCom>' + Virgula_Ponto(FormatFloat('0.0000',(StrToFloat(Ponto_Virgula(Trim(ConexaoBD.SQL_Selecao_Produtos_Nota.FieldByName('Selecao_Produto_Nota_Prod_Qtde').Text)))))) + '</qCom>';
                 Grava_Linha := Grava_Linha + '<vUnCom>' + Virgula_Ponto(FormatFloat('0.000',(StrToFloat(Ponto_Virgula(Trim(ConexaoBD.SQL_Selecao_Produtos_Nota.FieldByName('Selecao_Produto_Nota_Vlr_Unitario').Text)))))) + '</vUnCom>';
                 ...
                 ConexaoBD.SQL_Comunitario.SQL.Clear;
                 ConexaoBD.SQL_Comunitario.SQL.Add('UPDATE Cabeca_Cupons SET Cabeca_Cupom_Cliente_Codigo = ' +#39+ Trim(CPF_Cliente) +#39+ ', Cabeca_Cupom_Cliente_Status = ' +#39+ 'CFe:' + Trim(Numero_Nota) +#39+ ' WHERE Cabeca_Cupom_Numero = ' +#39+ Trim(ConexaoBD.SQL_Selecao_Produtos_Nota.FieldByName('Selecao_Produto_Nota_Cabeca').Text) +#39);
                 ConexaoBD.SQL_Comunitario.ExecSQL;

                 ConexaoBD.SQL_Selecao_Produtos_Nota.Next;
           End;

           //*** Limpa a Seleção dos Produtos ***
           ConexaoBD.SQL_Comunitario.SQL.Clear;
           ConexaoBD.SQL_Comunitario.SQL.Add('TRUNCATE TABLE Selecao_Produtos_Nota');
           ConexaoBD.SQL_Comunitario.ExecSQL;
           ...
           BTN_Emite_Nota.Caption := 'Imprimir XML';

           Seta();

           MSG_Erro('Geração do XML concluída, por favor, importe o arquivo localizado em: \\VIDAPERFEITA-PC\downloads\Importa_Nota_Fiscal_Paulista\CFe_' + Trim(Numero_Nota) + '.xml para o ELGIN-SAT e utilize a opção: ENVIAR DADOS VENDA. Após a transmissão, clique no Botão "Imprimir XML".');
        End;
        End
     Else If (Trim(XML_Gerado.Text) <> '') Then
        Begin

        Arquivo_XML := Trim(XML_Gerado.Text);
...
procedure TNotasFiscais01.DBGrid_SelecaoDblClick(Sender: TObject);
...
begin
    ...
    ConexaoBD.SQL_Comunitario.SQL.Clear;
    ConexaoBD.SQL_Comunitario.SQL.Add('DELETE FROM Selecao_Produtos_Nota WHERE Selecao_Produto_Nota_Cabeca = ' + #39+ Trim(DBGrid_Selecao.Fields[0].Text) +#39+ ' AND Selecao_Produto_Nota_Prod_Codigo = ' + #39+ Trim(DBGrid_Selecao.Fields[3].Text) +#39);
    ConexaoBD.SQL_Comunitario.ExecSQL;
...
procedure TNotasFiscais01.Rel_ValorKeyPress(Sender: TObject;
  var Key: Char);
begin
    If Key = #13 Then
      Begin
